 ...
 * POST /movies/chat
 * body: { message: String }
 * → génère une réponse LLM via Ollama REST
 */
app.post('/movies/chat', async (req, res) => {
  ...
  try {
    // Appel direct à Ollama (serveur lancé sur localhost:11434)
    const ollamaResp = await axios.post('http://localhost:11434/generate', {
      model: 'alpaca-7b',      // ou tout autre modèle léger
      prompt: [
        { role: 'system', content: 'Tu es un expert cinéphile, amical et précis.' },
        { role: 'user', content: message }
      ],
      max_tokens: 256
    });

    const answer = ollamaResp.data.choices?.[0]?.message?.content ?? '';
